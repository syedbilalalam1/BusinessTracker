...
export default function AddPurchaseDetails({
  ...
  const [purchase, setPurchase] = useState(
    editMode && purchaseData
      ? {
          ...
          productID: purchaseData.ProductID?._id,
  ...
  const handleSubmit = () => {
    const url = editMode 
      ? `http://localhost:4000/api/purchase/update/${purchaseData._id}`
    ...
    fetch(url, {
      ...
      .catch((err) => {
        ...
        alert(editMode ? "Failed to update purchase" : "Failed to add purchase");
      });
  };
